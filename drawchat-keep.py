...
def plot_customer_bubble_centered(df, customer_column, sow_column, ppd_column, volume_column, year_filter, bubble_scale=1.0, alpha=0.6, 
    ...
    def calculate_bubble_size_log(volume, min_volume, max_volume, volume_scale):
        if volume <= 0:
            return 20 * volume_scale
        ...
        min_size = 40 * volume_scale
        max_size = 180 * volume_scale
        return min_size + (normalized * (max_size - min_size))
    
    def calculate_bubble_size_sqrt(volume, min_volume, max_volume, volume_scale):
        if volume <= 0:
            return 20 * volume_scale
        ...
        min_size = 30 * volume_scale
        max_size = 120 * volume_scale
        return min_size + (normalized * (max_size - min_size))
